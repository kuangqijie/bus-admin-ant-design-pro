...
class BasicLayout extends React.PureComponent {
  ...
  render() {
    ...
    //只显示当前一级导航下的子菜单
    if(matchRes.length){
      var childRoute = matchRes[0].route.children
      if(childRoute && childRoute.length){
        filterMenu = childRoute;
        filterMenu.children = [];
      }else{
        //如果是首页 显示第一个导航子菜单
        if(matchRes[0].match.path == '/'){
          filterMenu = menuData.filter(item=> item.children)[0].children
        }
      }
    }else{
      //console.log(menuData)
      //未匹配到显示第一个导航子菜单
      filterMenu = menuData.filter(item=> item.children)[0].children
      //menuData = matchRes[0].route.children;
    }
